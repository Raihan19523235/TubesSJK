...
add_action( 'hoot_dynamic_cssrules', 'hootbiz_dynamic_cssrules', 5 );
 ...
 * @return array|string
 */
if ( !function_exists( 'hootbiz_user_style' ) ) :
function hootbiz_user_style( $key = false ){
	static $styles;
	...
	if ( empty( $styles ) ) {
		...
		$styles['logo_custom']          = apply_filters( 'hootbiz_logo_custom_text', hoot_sortlist( hoot_get_mod( 'logo_custom' ) ) );
		...
		$styles['smallwidgetmargin']    = ( intval( $styles['widgetmargin'] ) ) ?
		                                    ...
		                                    false;
		$styles = apply_filters( 'hootbiz_user_style', $styles );
	}
	if ( $key )
 ...
 * @access public
 */
function hootbiz_dynamic_cssrules() {
