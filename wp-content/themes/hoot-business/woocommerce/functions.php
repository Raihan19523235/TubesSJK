 ...
 * @return void
 */
if ( !function_exists( 'hootbiz_woo_setup' ) ) :
function hootbiz_woo_setup() {
...
add_action( 'wp', 'hootbiz_woo_setup' );
 ...
 * @return void
 */
if ( !function_exists( 'hootbiz_woo_register_sidebars' ) ) :
function hootbiz_woo_register_sidebars() {
	hoot_register_sidebar(
		array(
			'id'          => 'hoot-woo-sidebar-primary',
			'name'        => _x( 'Woocommerce Primary Sidebar', 'sidebar', 'hoot-business' ),
			'description' => __( 'The primary sidebar for woocommerce pages.', 'hoot-business' )
		)
	);
	hoot_register_sidebar(
		array(
			'id'          => 'hoot-woo-sidebar-secondary',
			'name'        => _x( 'Woocommerce Secondary Sidebar', 'sidebar', 'hoot-business' ),
			'description' => __( 'The secondary sidebar for woocommerce pages (if you are using a 3 column layout with 2 sidebars).', 'hoot-business' )
		)
	);
}
endif;
add_action( 'widgets_init', 'hootbiz_woo_register_sidebars' );
 ...
 * @return array
 */
if ( !function_exists( 'hootbiz_woo_attr_sidebar' ) ) :
function hootbiz_woo_attr_sidebar( $attr, $context ) {
...
add_filter( 'hoot_attr_sidebar', 'hootbiz_woo_attr_sidebar', 11, 2 );
 ...
 * @return array
 */
if ( !function_exists( 'hootbiz_woo_layout' ) ) :
function hootbiz_woo_layout( $sidebar ) {
	...
	$forcenosidebar = apply_filters( 'hootbiz_woo_pages_force_nosidebar', true );
...
add_filter( 'hootbiz_layout', 'hootbiz_woo_layout' );
 ...
 * @param string $context
 */
if ( !function_exists('hootbiz_woo_meta_info_blocks_display') ) :
function hootbiz_woo_meta_info_blocks_display( $display, $context ) {
	if ( is_woocommerce() || is_cart() || is_checkout() || is_account_page() )
		$display = array();
	return $display;
}
endif;
add_filter( 'hoot_meta_info', 'hootbiz_woo_meta_info_blocks_display', 10, 2 );
 ...
 * @return void
 */
if ( !function_exists( 'hootbiz_woo_set_queried_object' ) ) :
function hootbiz_woo_set_queried_object( $q ){
	if ( $q->is_main_query() )
		$r = get_queried_object();
}
endif;
add_action( 'pre_get_posts', 'hootbiz_woo_set_queried_object', 9 );
 ...
 * @return void
 */
if ( !function_exists( 'hootbiz_woo_loop_hide_product_meta' ) ) :
function hootbiz_woo_loop_hide_product_meta() {
